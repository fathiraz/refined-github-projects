...
function ProcessCard({
  ...
  entry: ProcessEntry
  onDismiss: (processId: string, isDone: boolean) => void
}) {
  ensureTippyCss()
  ...
  const hasFailures = isDone && entry.failedItems && entry.failedItems.length > 0
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flexShrink: 0, pt: '2px' }}>
        {isDone ? (
          <Box sx={{ color: hasFailures ? 'attention.fg' : 'success.fg' }}>
            <CheckIcon size={20} />
          </Box>
        ) : entry.paused ? (
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        {hasFailures && (
          <Box sx={{ mt: 2 }}>
            ...
            >
              {entry.failedItems!.length} failed {showFailed ? '\u25BE' : '\u25B8'}
            </Button>
            ...
            {showFailed && (
              ...
              >
                <ActionList sx={{ py: 0 }}>
                  {entry.failedItems!.map((item) => (
                    <ActionList.Item key={item.id} disabled sx={{ py: 1, px: 2 }}>
                      <ActionList.LeadingVisual>
                        <Box sx={{ color: 'danger.fg', fontSize: 0 }}>&#x2715;</Box>
                      </ActionList.LeadingVisual>
                ...
                >
                  {entry.retryContext && (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        const failedIds = entry.failedItems!.map((f) => f.id)
                        sendMessage(
                          ctx.messageType as any,
                          {
                            ...ctx.data,
                            itemIds: failedIds,
                          } as any,
                  ...
                  <Button
                    ...
                    variant="invisible"
                    onClick={() => {
                      const lines = entry.failedItems!.map((f) => `- ${f.title}: ${f.error}`)
                      const text = `Failed items (${entry.failedItems!.length}):\n${lines.join('\n')}`
        ...
        {!isDone && (entry.status || entry.detail) && (
          ...
          >
            ...
            <Box
              sx={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '3px' }}
            >
